...
class TestAppExceptionHandler:
    async def test_app_exception_returns_correct_shape(
        ...
        response = await exception_client.get("/test/app-exception")
    ...
    async def test_app_exception_status_code_is_respected(
        ...
        response = await exception_client.get("/test/conflict")

        assert response.status_code == 409
        assert response.json()["error_code"] == "DUPLICATE_EMAIL"

    async def test_app_exception_403(self, exception_client: AsyncClient):
        @app.get("/test/forbidden")
        async def _():
            raise AccessDeniedError()

        response = await exception_client.get("/test/forbidden")
    ...
    async def test_app_exception_body_has_exactly_two_keys(
        ...
        response = await exception_client.get("/test/shape-check")

        assert set(response.json().keys()) == {"error_code", "detail"}


class TestValidationErrorHandler:
    async def test_validation_error_returns_422_shape(
        self, exception_client: AsyncClient
    ):
...
class TestUnhandledExceptionHandler:
    async def test_unhandled_exception_returns_500(self, exception_client: AsyncClient):
        @app.get("/test/unhandled")
        async def _():
            raise RuntimeError("secret internal error message")

        response = await exception_client.get("/test/unhandled")
    ...
    async def test_unhandled_exception_does_not_leak_detail(
        ...
        response = await exception_client.get("/test/unhandled-leak")
    ...
    async def test_unhandled_exception_body_shape(self, exception_client: AsyncClient):
...
class TestRedisErrorHandler:
    async def test_redis_error_returns_503(self, exception_client: AsyncClient):
        ...
        response = await exception_client.get("/test/redis-error")
    ...
    async def test_redis_error_does_not_leak_detail(
        self, exception_client: AsyncClient
    ):
        ...
        response = await exception_client.get("/test/redis-leak")
