...
class TestGetCache:
    ...
    async def test_get_cache_returns_none_on_redis_error(self):
    ...
    async def test_get_cache_returns_none_on_connection_error(self):
        redis = make_redis(get_side_effect=ConnectionError("refused"))
        result = await get_cache(redis, "my-key")

        assert result is None

    async def test_get_cache_returns_none_on_timeout(self):
        redis = make_redis(get_side_effect=TimeoutError("timed out"))
        result = await get_cache(redis, "my-key")

        assert result is None

    async def test_get_cache_calls_redis_with_correct_key(self):
        redis = make_redis()
...
class TestSetCache:
    async def test_set_cache_calls_redis_set(self):
        redis = make_redis()
    ...
    async def test_set_cache_swallows_connection_error(self):
...
class TestDeleteCache:
    ...
    async def test_delete_cache_swallows_redis_error(self):
        redis = make_redis(delete_side_effect=RedisError("delete failed"))
        # Must not raise
        await delete_cache(redis, "my-key")


class TestGetCacheCritical:
    async def test_get_cache_critical_returns_value(self):
    ...
    async def test_get_cache_critical_raises_on_redis_error(self):
    ...
    async def test_get_cache_critical_raises_on_connection_error(self):
        redis = make_redis(get_side_effect=ConnectionError("refused"))

        with pytest.raises(ConnectionError):
            await get_cache_critical(redis, "critical-key")

    async def test_get_cache_critical_raises_on_timeout(self):
        redis = make_redis(get_side_effect=TimeoutError("timed out"))

        with pytest.raises(TimeoutError):
            await get_cache_critical(redis, "critical-key")

    async def test_get_cache_critical_calls_redis_with_correct_key(self):
        redis = make_redis()
...
class TestSetCacheCritical:
    async def test_set_cache_critical_calls_redis_set(self):
        redis = make_redis()
        await set_cache_critical(redis, "critical-key", "value")

        redis.set.assert_called_once_with("critical-key", "value", ex=None)

    async def test_set_cache_critical_passes_ttl(self):
        redis = make_redis()
        await set_cache_critical(redis, "critical-key", "value", ex=60)

        redis.set.assert_called_once_with("critical-key", "value", ex=60)

    async def test_set_cache_critical_raises_on_redis_error(self):
        redis = make_redis(set_side_effect=RedisError("write failed"))

        with pytest.raises(RedisError):
            await set_cache_critical(redis, "critical-key", "value")

    async def test_set_cache_critical_raises_on_connection_error(self):
        redis = make_redis(set_side_effect=ConnectionError("refused"))
...
class TestBehaviouralContract:
    async def test_optional_wrapper_never_raises_redis_error(self):
        """
    ...
    async def test_critical_wrappers_always_raise_redis_error(self):
        """
