...
class TestSuccessfulActivation:
    async def test_returns_204_on_success(
        ...
        response = await integration_client.patch(
            f"{ENDPOINT}/{deactivated.public_id}/activation",
            headers=headers,
        )
...
class TestAuth:
    async def test_unauthenticated_returns_401(
        ...
        response = await integration_client.patch(
            f"{ENDPOINT}/{teacher.public_id}/activation"
        )
...
class TestNotFound:
    async def test_unknown_public_id_returns_404(
        ...
        response = await integration_client.patch(
            f"{ENDPOINT}/{uuid.uuid4()}/activation",
            headers=headers,
        )
...
class TestIntegrityErrors:
    ...
    async def test_invalid_status_transition_returns_403(
        ...
        response = await integration_client.patch(
            f"{ENDPOINT}/{pending.public_id}/activation",
            headers=headers,
        )
