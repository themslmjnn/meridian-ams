...
class UserServiceAdmin:
    ...
    async def register_user(
        ...
        _, hashed_activation_token = generate_activation_token()

        activation_token_expires_at = datetime.now(UTC) + timedelta(
            hours=settings.ACTIVATION_TOKEN_EXPIRES_HOURS
        )
        if (
        ...
        try:
            ...
            new_user_session = UserSession(credentials_id=new_user_credentials.id)
